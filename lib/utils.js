...
export function computeShippingCost(subtotal, distanceKm, destCity, dynamicConfig = null) {
  ...
  switch (frete.modelo) {
    ...
    case 'VALOR': {
      ...
      if (!tier) return { cost: 'pending', note: '⚠️ Faixa de frete não configurada para este valor.' };
    ...
    case 'KM': {
      if (distanceKm == null || isNaN(distanceKm)) {
        return { cost: 'pending', note: '⚠️ Calcule o frete pelo CEP para ver o valor.' };
      }
    ...
    case 'CIDADE': {
      ...
      if (!dest) return { cost: 'pending', note: '⚠️ Informe o CEP para calcular.' };
...
function _fallback(subtotal, destCity) {
  ...
  if (dest && dest !== originCity) {
    return { cost: 'pending', note: '⚠️ Frete a combinar com o vendedor.' };
  }
